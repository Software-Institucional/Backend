  ...
  async updateme(user: User): Promise<User> {
    const updatedUser = await this.prisma.user.update({
      ...
      data: {
        ...
        imgUrl: user.imgUrl,
        password: user.password,
        simatuser: user.simatuser, // ojo aquí, NO simatusers
        simatpass: user.simatpass,
        updatedAt: new Date(),
      },
    });
    return new User(
      ...
      updatedUser.createdAt,
      updatedUser.updatedAt,
      updatedUser.imgUrl ?? undefined,
      updatedUser.simatuser ?? undefined,
      updatedUser.simatpass ?? undefined,
    );
  }
}
